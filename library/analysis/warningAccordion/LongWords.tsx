...
function LongWords(props: { content: any }) {
  ...
  const allFreq = Object.entries(longWords).slice(
    indexOfFirstPost,
    indexOfLastPost
  );
  ...
  return (
    <>
      {longWordsHere == 1 && (
        <Accordion.Item>
          <Accordion.Header>
            {listLongWords.length == 1 ? (
              <>1 unikt langt</>
            ) : (
              <>{longWordCounter} unike lange</>
            )}{' '}
            ord
          </Accordion.Header>
          <Accordion.Content>
            ...
            Ifølge lesbarhetsindeksen Liks anses ord med over seks bokstaver som
            lange -{' '}
            <Link
              target="_blank"
