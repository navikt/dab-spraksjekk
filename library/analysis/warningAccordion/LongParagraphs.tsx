...
function LongParagraphs(props: { content: any }) {
  ...
  rawcontent = rawcontent
    .split('\n')
    .map((l: string) =>
      l.length > 0 && !['.', ':', '!', '?'].includes(l.slice(-1)) ? l + '.' : l
    )
    .join('\n');
    ...
    .sort(function (
      a: {
        replace: (
          ...
          new (): any;
          split: {
            (arg0: string): { (): any; new (): any; length: number };
            new (): any;
          };
        };
      },
  ...
  for (let i in paragraphs) {
    const sentencesInParagraphs = paragraphs[i]
      .replace(/([.?!])\s*(?=[A-Z])/g, '$1|')
      .split('|');
  ...
  const longParagraphs = myLongParagraphs.filter(
    (item) => item.length > paragraphLength
  );

  // Pagination pages
  const indexOfLastPost = page * pageSize;
  const indexOfFirstPost = indexOfLastPost - pageSize;
  const allFreq = Object.entries(longParagraphs).slice(
    indexOfFirstPost,
    indexOfLastPost
  );
      ...
      {longParagraphHere != 0 && (
        <Accordion.Item>
          ...
          <Accordion.Content>
            ...
            "Et avsnitt bør ha ett hovedbudskap og ikke ha mer enn to til tre
            setninger." -{' '}
            <Link
              target="_blank"
                ...
                return (
                  <li key={wordFreq[0]} className="språkhjelp-pb-5">
                    {expanded[wordFreq[0]] ? (
                      ...
                      <>"{wordFreq[1].match(firstSentenceRegex)[0]} </>
                    )}
                    <Button
                      size="xsmall"
                      variant="secondary"
