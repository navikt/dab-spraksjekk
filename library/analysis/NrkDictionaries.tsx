...
function NrkDictionaries(props: { content: any }) {
  ...
  if (keyword !== '') {
    const results = ordliste.nrkordliste.filter((ordliste) => {
      return keyword
        .toLowerCase()
        .match('\\b' + ordliste.ord.toLowerCase() + '\\b');
    });
  ...
  return (
    <>
      {ordlisteVisResultater != 0 && (
        <Accordion.Item>
          <Accordion.Header>
            {ordlisteResultater.length == 1 ? (
              <>1 mulig støtende ord</>
            ) : (
              <>{ordlisteResultater.length} mulige støtende ord</>
            )}
          </Accordion.Header>
          <Accordion.Content>
            ...
            Ord i teksten som kan være støtende, eller som bør brukes med
            varsomhet:
              ...
              {ordlisteResultater.map((ordliste, i) => (
                <Accordion.Item key={ordliste.id}>
                  ...
                  <Accordion.Content className="språkhjelp-inner-accordion-content">
                    ...
                    {
                      <Link target="_blank" href={ordliste.lenke}>
                        {ordliste.kilde}
                        <ExternalLinkIcon />
                      </Link>
                    }
