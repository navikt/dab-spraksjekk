...
function LongSentences(props: { content: any }) {
  ...
  const longSentences = myLongSentences.filter(
    (item) => item.length > sentenceLength
  );
  ...
  const allFreq = Object.entries(longSentences).slice(
    indexOfFirstPost,
    indexOfLastPost
  );
      ...
      {longSentenceHere != 0 && (
        <Accordion.Item>
          ...
          <Accordion.Content className="språkhjelp-remove-accordion-padding-bottom">
            ...
            Ifølge studier kan setninger med over 20 ord anses som vanskelige å
            lese -{' '}
            <Link
              target="_blank"
              ...
              {allFreq.map((wordFreq: [string, string]) => {
                return (
                  <li key={wordFreq[0]} className="språkhjelp-pb-5">
                    "{wordFreq[1]}"{' '}
                    <b>({wordFreq[1].split(/\s+/).length}&nbsp;ord)</b>
                  </li>
                );
              })}
            </ul>
